...
- **H3 (partial)** — `HortayBackend` expect/actual in commonMain. Android actual delegates to `TdClient` + `CountryRepository` + `ChannelActionsRepository`; iOS actual stubs everything. Surface covers auth (stage / error / phone / code / password / recovery), countries, and channel/user lookups (channelInfo / setMuted / join / leave / userProfile). Three screens moved to commonMain: `AuthScreen`, `ChannelInfoSheet`, `HiddenChannelsScreen`. Current file split: ~103 androidMain / ~92 commonMain / ~12 iosMain.

**Still pending:**
- **H3 (remainder)** — `UserProfileSheet` (depends on H4 clipboard/toast/share), `CommentsScreen` (~830 lines, drags `CommentsRepository` surface), `MainScaffold` + dialogs + `NavOverlayRenderer` + `TabContentSwitcher`, `TimelineScreen` + `ChannelScreen` + ViewModels, `SettingsScreen` + `AutoDownloadScreen`. Each adds more methods to `HortayBackend`.
- **H4** — Android system services (Clipboard, Share, Toast, ImageBitmap decode, status bar) expect/actual. Unblocks `UserProfileSheet`, `LinkActionsSheet`, `MediaShareActions`, `PostActions`, `MinithumbImage`, `Theme`, `HortayUriHandler`, `LinkAwareScaffold`.
- **H5** — iOS UI parity ship: replace `MainViewController`'s parallel renderer with the real `WebModeScaffold` tree from commonMain.
- Bulk UI move depends on H3 (remainder) — PostCard's transitive graph still touches TDLib repos. Without that abstraction, individual UI files can't move because they import androidMain helpers (`MediaCache.observe`, `CustomEmojiRepository`, etc.).
