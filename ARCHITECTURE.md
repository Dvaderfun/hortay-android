...
- **`:shared`** — KMP library module with Compose Multiplatform UI. Repositories, ViewModels, all UI, data layer, SQLDelight DAOs. DI via Koin 4.x (`shared/src/commonMain/kotlin/dev/lyo/hortay/di/`). Plugin: `com.android.kotlin.multiplatform.library` + `org.jetbrains.kotlin.multiplatform` + `org.jetbrains.compose`. Source sets: `commonMain` (shared across all targets), `androidMain` (Android-specific incl. TDLib + ExoPlayer), `iosMain` (iOS actuals + TDLib cinterop). JVM 21.
...
**Namespace split.** `:shared` namespace = `dev.lyo.hortay`. `:androidApp` namespace = `dev.lyo.hortay.app` (avoids R-class collision). All user-facing strings use CMP `Res.string.*` (no `R.string.*` references remain). Manifest uses FQCN `android:name="dev.lyo.hortay.HortayApp"` because `HortayApp` and `MainActivity` live in `androidApp/src/main/kotlin/dev/lyo/hortay/` — manifest's namespace-relative resolution would otherwise look in `dev.lyo.hortay.app.*`.
...
**Code distribution:** ~40 files in `androidMain`, ~200 in `commonMain`, ~30 in `iosMain`. Every UI screen lives in commonMain. Nav substrate is nav3's `NavDisplay` mounted inside `MainScaffold` / `WebModeScaffold`. androidMain is the irreducible core: `tdlibModule` + TDLib-bound repositories + `LogoutCleanup` + platform actuals (Coil disk cache, status bar, language picker, data-saver probe). The platform-agnostic surface lives on `HortayBackend` (commonMain expect/actual) plus `Local*` CompositionLocal slots.
...
| Web-mode SQL portability | `shared/src/commonMain/sqldelight/.../web/db/*.sq` | All upserts via `INSERT OR IGNORE` + `UPDATE` — **not** `ON CONFLICT DO UPDATE`. Android 8/9 SQLite < 3.24. FTS5 skipped. |
...
- ❌ Room. SQLDelight 2.3.2 owns `web.db`; TDLib owns its own. **Revisit:** never (no shared scope).
- ❌ Adding a general HTTP client beyond the current stack. Ktor handles web-mode `t.me/s/` pipeline + custom-emoji JSON; Coil handles images. **Revisit:** if a typed REST backend joins the stack (none planned).
...
- ✅ SQLDelight 2.3.2 for `web.db` only. TDLib mode runs without a DB.
