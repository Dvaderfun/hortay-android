 ...
 *
 * Throttling: [TdApi.SearchPublicChat]/[TdApi.SearchPublicChats] hit the server when
 * the chat isn't already cached. Curated hydration fans out across ~20 handles on
 * sheet open, so resolves are serialized behind [throttle] at ~1/sec — the same
 * ceiling the guest→TDLib subscription migration uses (tdlib/td#743 flood-control).
 * Already resolved handles short-circuit from [resolveCache] with no RPC, so a
 * re-opened sheet paints instantly.
 *
...
class ChannelDiscoveryRepository(
    ...
    // Atomic persistent map (lock-free CAS) — KMP stand-in for the JVM
    // ConcurrentHashMap of the original.
    private val resolveCache = atomic(persistentMapOf<String, DiscoverChannel>())
    ...
    suspend fun resolve(username: String): DiscoverChannel? {
        ...
        resolveCache.value[key]?.let { return it }
        val chat = throttled {
            runCatching { td.send(TdApi.SearchPublicChat(username.removePrefix("@").trim())) }
                .warnUnlessCancelled(TAG, "resolve($username)")
                .getOrNull()
        } ?: return null
        return toCard(chat)?.also { card ->
            while (true) {
                val current = resolveCache.value
                if (resolveCache.compareAndSet(current, current.put(key, card))) break
            }
        }
    }
    ...
    fun clear() {
        resolveCache.value = persistentMapOf()
    }
