 ...
 *
 * Single extraction path: every snapshot in the archive comes from a real [TdApi.MessageContent]
 * (UpdateMessageContent's `newContent`, or the initial mapping in handleNewMessage). Deriving meta
 * from a mapped TimelinePost was removed — it stripped entities and produced a different
 * `content_hash` for the same logical post, spamming the archive with "phantom" edits.
 */
object TdlibContentMetaExtractor {
    ...
    /**
     * Extracts an [ArchivedMediaRef] from a media-bearing [TdApi.MessageContent]. Returns null for
     * text-only / poll-without-banner / call / venue / contact / location / sticker content — all
     * carry no downloadable media file. Stickers are intentionally excluded (lifecycle tied to the
     * StickerSet, not the message). The largest [TdApi.PhotoSize] is selected for photos.
     */
