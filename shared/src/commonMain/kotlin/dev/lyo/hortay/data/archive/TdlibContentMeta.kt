...
/**
 * Extracted metadata from a TDLib `TdApi.MessageContent` that can be serialized to disk via
 * [ContentBlobCodec] (ProtoBuf).
 *
 * Kept deliberately narrow — only the fields needed to render a revision diff in
 * [dev.lyo.hortay.ui.archive.PostRevisionSheet] and to populate the archive search index.
 *
 * [entitiesJson] / [forwardJson] / [replyJson] are serialized as JSON strings so the schema
 * can evolve without a ProtoBuf field-number migration.
 *
 * ProtoBuf field numbers are pinned via [ProtoNumber] so old blobs decoded with a newer
 * schema (additional optional fields) stay readable. New fields MUST use a fresh number and
 * tolerate default values for existing rows.
 */
@Immutable
@Serializable
data class TdlibContentMeta(
    ...
    /**
     * Structured media reference for the captured snapshot. Null when post carries no media
     * (text-only, poll without photo banner, etc.). Carries persistent remote identifiers +
     * optional local-archive SHA so the revision sheet can render the media even after the
     * original message is deleted server-side.
     */
