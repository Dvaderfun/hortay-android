 ...
 *
 * Mirrors the type-by-type dispatch in [TdlibContentMetaExtractor]'s media extraction — kept as a
 * thin separate helper so [ArchiveRepository] stays TDLib-free (the file copy is invoked by
 * capture-site code in PostsRepository / CommentsRepository before they hand the resulting SHA to
 * the repository).
 *
 * Returns `null` for text-only / poll-without-banner / sticker / venue / contact content.
 */
