...
/**
 * Background-gradient colours for the profile photo backdrop, as opaque ARGB. Picks the
 * dark- or light-theme variant; empty when TDLib supplied none (the caller then renders
 * the brand fallback brush).
 */
...
/**
 * Avatar-ring colours, as opaque ARGB. Prefers `storyColors` (the gradient Telegram draws
 * around the photo); falls back to `paletteColors` when the user's accent has no story
 * gradient. Empty when neither is set.
 */
 ...
 * colour (`id == -1`), the id is unknown, or TDLib supplied no colours — the caller then
 * paints the brand fallback brush.
 */
 ...
 * just keep the last snapshot in an atomic reference and look colours up by id.
 *
 * Cleared on `loggedOut` (hard rule: every session-scoped state holder clears on logout)
 * so a fresh sign-in on another account can't read the previous palette.
 */
class ProfileAccentRegistry : ProfileAccentResolver {
    private val byId = atomic<Map<Int, TdApi.ProfileAccentColor>>(emptyMap())
    ...
    fun ingest(colors: Array<TdApi.ProfileAccentColor>) {
        byId.value = colors.associateBy { it.id }
    }
    ...
    private fun color(accentId: Int): TdApi.ProfileAccentColor? =
        if (accentId < 0) null else byId.value[accentId]
