 ...
 *
 * Media downloads use a longer grace (600 ms in MediaProgressIndicator) —
 * file IO legitimately takes longer than data hydration and a faster grace
 * would strobe progress discs on healthy connections. The two values are
 * calibrated to different latency budgets, not duplicates.
 *
 ...
 * animations" / iOS Reduce Motion) — when there's no transition to hide
 * behind, grace is 0 and the skeleton paints on the first Loading frame.
 */
 ...
 *     iOS Reduce Motion) → returns 0. The destination appears without a
 *     transition; any grace would just be a delay before painting feedback
 *     the user needs sooner.
 *
