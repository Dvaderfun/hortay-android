 ...
 *
 * Media refcount: when a snapshot carrying a [TdlibContentMeta.mediaRef] with a non-null
 * [ArchivedMediaRef.localArchiveSha] is evicted, the underlying file's refcount is decremented —
 * otherwise orphaned media accumulates. The sweep grabs blobs of soon-to-be-evicted rows, decodes
 * them to recover the SHAs, then releases the refs after the SQL DELETE.
 */
class ArchiveSweep(
    ...
    suspend fun run() {
        ...
        // Each select-blobs+delete pair runs inside one transaction so a concurrent capture can't
        // slip a write between the SELECT and the DELETE. That matters most for the cap branch:
        // deleteByCap recomputes MAX(id) at delete time, so an interleaved insert would shift the
        // cutoff and evict a row whose blob we never collected — its media file would then never
        // have its refcount released and leak on disk.
