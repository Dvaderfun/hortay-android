 ...
 * space [androidx.compose.foundation.lazy.LazyListLayoutInfo] uses (item.offset
 * + item.size for the bottom; viewportStartOffset/viewportEndOffset, optionally
 * tightened by before/afterContentPadding).
 *
 ...
 * tops out at 60% of its own span, and the 28 dp [dev.lyo.hortay.ui.composables.skeleton.UnreadBoundaryRow]
 * sitting above the boundary post in OldestUnreadFirst eats 28 px more — pushing
 ...
 * [androidx.compose.foundation.lazy.LazyListLayoutInfo], and close the remaining gap with
 * one [scrollBy]. This function is just that gap. "Measure reality, nudge by the
 * difference" can't drift from undocumented internals because it reads the real geometry
 * every time.
 *
 ...
 * [LazyListState.layoutInfo]), which doesn't exist until the first layout pass.
