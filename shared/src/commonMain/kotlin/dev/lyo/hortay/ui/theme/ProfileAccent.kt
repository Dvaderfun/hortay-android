...
/**
 * Resolver for per-user Telegram profile accent colours. Seeded once at the entry point
 * from the Koin-provided `ProfileAccentRegistry`; defaults to the always-fallback resolver
 * so guest mode, auth and previews render the brand cover.
 *
 * Why a CompositionLocal and not a parameter: the resolver is consumed on two unrelated
 * surfaces (Settings hero, user-profile sheet) and would otherwise thread through several
 * composables. Same mitigation as `LocalReadCursors` / `LocalMediaCache`.
 */
 ...
 * which produced muddy in-between tones. Matches how Telegram fills its profile header with the
 * peer colours.
 */
@Composable
